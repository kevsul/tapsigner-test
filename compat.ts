...
function CT_bip32_derive(
  ...
  if (master_priv_pub.length === 32) {
    master = Factory.fromPrivateKey(master_priv_pub, chain_code); // master_priv_pub :: private_key
  } else {
    master = Factory.fromPublicKey(master_priv_pub, chain_code); // master_priv_pub :: public_key
  }
